...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeContent(self, filter):
        ...
        if not "class" in result or result["class"] == []:
            result["class"] = [
                ...
                {"type_name": "体育", "type_id": "体育超清"},
                {"type_name": "球星", "type_id": "球星"},
                {"type_name": "教育", "type_id": "中小学教育"},
                {"type_name": "幼儿教育", "type_id": "幼儿教育"},
        ...
        if cid == "动态":
            ...
            for vod in vodList:
                if vod["type"] != "DYNAMIC_TYPE_AV":
                    continue
                vid = str(
                    vod["modules"]["module_dynamic"]["major"]["archive"]["aid"]
                ).strip()
                remark = vod["modules"]["module_dynamic"]["major"]["archive"][
                    "duration_text"
                ].strip()
                title = self.removeHtmlTags(
                    vod["modules"]["module_dynamic"]["major"]["archive"]["title"]
                ).strip()
            ...
            if page < data["data"]["page"]["count"]:
                ...
                pagecount = page
            if page == 1:
                videos = [{"vod_id": f"UP主&&&{cid}", "vod_name": "播放列表"}]
    ...
    def playerContent(self, flag, pid, vipFlags):
        ...
        result["playUrl"] = ""
        result["url"] = (
            f"http://127.0.0.1:9978/proxy?do=py&type=mpd&cookies={cookies}&url={quote(url)}&aid={aid}&cid={cid}&thread={thread}"
        )
        result["header"] = self.header
        result["danmaku"] = "https://api.bilibili.com/x/v1/dm/list.so?oid={}".format(
            cid
        )
...
if __name__ == "__main__":
    ...
    time.sleep(1)
    # print("\n-------------------获取视频列表测试------------------------------")
    # rsp = spider.detailContent(["BV1xx411c7mu"])  # 示例BV号
    # print(rsp)
    # time.sleep(1)
    # print("\n-------------------解析视频地址测试------------------------------")
    # id = rsp["list"][0]["vod_play_url"].split("#")[0].split("$")[1]  # 获取第一个播放地址
